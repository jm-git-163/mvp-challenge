...
- 2026-04-24 fix(share): KakaoTalk 공유 버튼 → Play Store "카카오톡 다운로드" 페이지 리다이렉트 버그 완전 제거. (a) utils/shareVideo.ts 의 `window.location.href = scheme` / intent:// fallback / https new-tab fallback / 자동 `navigator.share({text})` 재호출 전부 삭제 — 이전엔 download 직후 Android 에서 자동으로 다시 share sheet 를 띄워 Kakao 탭 시 file MIME 없이 dispatcher 가 Play Store 로 튕겼음. (b) utils/share.ts sharePlatform 에 iOS ONLY 네이티브 file share 경로 추가 — iOS Safari 는 `navigator.share({files:[mp4]})` 로 카톡 채팅 선택창을 직접 엶. Android/desktop 은 다운로드+캡션복사+토스트 ("카톡/인스타 직접 열고 갤러리에서 선택") 로 일관. 어떤 플랫폼에서도 scheme/intent/https 마케팅 페이지 이동 없음.
