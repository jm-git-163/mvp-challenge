...
## 🎬 배경 비디오 (PIXABAY-VIDEO 2026-05-02 — 자동 다운 완료)

`public/templates/<id>/bg-loop.mp4` 및 `public/bg/*.mp4` — Pixabay Videos API 로 자동 다운로드 완료.
- 4 템플릿 무빙 배경: squat-master, idol-dance, news-anchor, emoji-explosion
- 4 범용 BG: sky-loop, particles-loop, intro-burst, outro-celebration
- 모두 5MB 안팎의 small 화질, Pixabay Content License (docs/ATTRIBUTIONS.md 참조)
- `video_bg` 레이어가 `bg_mesh` 위·`cam_feed` 아래 zIndex 5~7 에서 합성 (opacity 0.45~0.50, blur 2, blendMode screen)
- 교체 시: 동일 경로에 mp4 덮어쓰면 됨 (재실행 불필요)
