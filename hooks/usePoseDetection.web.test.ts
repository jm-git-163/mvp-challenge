...
describe('usePoseDetection (web)', () => {
  ...
  it('프로덕션에서 실패 시 camera-not-ready 에러를 설정한다', async () => {
    ...
    expect(mockState.data[4]).toBe('camera-not-ready'); // error state
    expect(mockState.data[0]).toBe('error');            // status state
  });
