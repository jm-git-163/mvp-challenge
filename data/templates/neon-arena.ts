...
export const neonArena: Template = {
  ...
  layers: [
    ...
    // FIX-Z25: 사이버펑크 도시 스카이라인 — 상단/하단 띠로 배치해 얼굴존(540~1200) 회피
    { id: 'bg_skyline_top', type: 'floating_shapes',  zIndex: 4,  opacity: 0.75, enabled: true, props: { shapes: ['cube', 'cube', 'pyramid', 'cube', 'pyramid'], yBand: [80, 380], preset: 'skyline' } },
    { id: 'bg_skyline_bot', type: 'floating_shapes',  zIndex: 5,  opacity: 0.7,  enabled: true, props: { shapes: ['cube', 'pyramid', 'cube'], yBand: [1500, 1780], preset: 'skyline' } },
    // FIX-Z25: 네온 튜브 램프 줄 (상·하 엣지)
    { id: 'bg_neon_tube_t', type: 'orbiting_ring',    zIndex: 6,  opacity: 0.55, enabled: true, props: { color: '#FF2D95', radiusPx: 720, widthPx: 3, periodSec: 14, centerY: 160 } },
    { id: 'bg_neon_tube_b', type: 'orbiting_ring',    zIndex: 7,  opacity: 0.55, enabled: true, props: { color: '#00E0FF', radiusPx: 720, widthPx: 3, periodSec: 16, centerY: 1760 } },
    // FIX-Z25: 땀방울/스파크 파티클 (피트니스 테마)
    { id: 'bg_sweat',       type: 'particle_ambient', zIndex: 8,  opacity: 0.55, enabled: true, props: { preset: 'electric_blue_rise', count: 30 } },
    { id: 'bg_noise',       type: 'noise_pattern',    zIndex: 9,  opacity: 0.08, enabled: true },
    ...
    // FIX-Z25: center → top-center (fontSize 140→110) 얼굴 회피
    { id: 'intro_title',    type: 'kinetic_text',     zIndex: 11, opacity: 1,    enabled: true, props: { text: 'FITNESS MODE', fontSize: 110, color: '#FF2D95', strokeColor: '#00E0FF', strokeWidth: 10, mode: 'pop', staggerMs: 70, position: 'top-center', startMs: 200 }, activeRange: { startSec: 0, endSec: 2.8 } },
    ...
    { id: 'hud_counter',    type: 'counter_hud',      zIndex: 60, opacity: 1,    enabled: true, props: { target: 10, format: '{n} / 10', fontSize: 72, fontFamily: '"JetBrains Mono"', position: 'bottom-center' } },
    ...
    // FIX-Z25: center → top-center 얼굴 회피
    { id: 'outro_title',    type: 'kinetic_text',     zIndex: 76, opacity: 1, enabled: true, props: { text: 'CHALLENGE COMPLETE', fontSize: 80, color: '#FF2D95', strokeColor: '#00E0FF', strokeWidth: 8, mode: 'pop', position: 'top-center', startMs: 17100, staggerMs: 55 }, activeRange: { startSec: 17, endSec: 20 } },
